...
const requiredDocs = [
  "docs/phase-20-private-beta-round-1-feedback-intake-scope-lock.md",
  "docs/private-beta-round-1-test-plan.md",
  "docs/private-beta-tester-cohort-plan.md",
  "docs/private-beta-feedback-intake-workflow.md",
  "docs/private-beta-issue-triage-matrix.md",
  "docs/private-beta-regression-decision-matrix.md",
  "docs/private-beta-round-1-results-report-template.md",
  "docs/private-beta-feedback-tracker-template.md",
  "docs/phase-20-private-beta-round-1-feedback-intake.md",
];

const requiredAssertions = [
  { name: "private beta", patterns: [/private beta/i] },
  { name: "local-first", patterns: [/local-first/i] },
  { name: "no public release", patterns: [/no public release/i, /not.*public release/i] },
  ...
  { name: "no token sharing", patterns: [/no token sharing/i, /must not.*tokens/i, /do not paste tokens/i] },
  { name: "issue severity definitions", patterns: [/blocker/i, /high/i, /medium/i, /low/i, /cosmetic/i] },
  { name: "issue priority definitions", patterns: [/P0/i, /P1/i, /P2/i, /P3/i] },
  { name: "safety/data escalation", patterns: [/safety.*data/i, /data loss/i, /token exposure/i] },
];
...
const negativeBoundaryContext =
  /\b(no|not|must not|does not|do not|never|forbidden|absent|blocked|without|excludes|not implemented|not included|not available|planning only|not a|did not|was not|was added)\b/i;
...
for (const doc of requiredDocs) {
  ...
  checks.push({
    name: `required Phase 20 doc: ${doc}`,
    passed: exists,
...
checks.push({
  name: "required script: beta:verify:intake",
  passed: Boolean(packageJson.scripts?.["beta:verify:intake"]),
  detail: packageJson.scripts?.["beta:verify:intake"] ?? "missing",
});
...
for (const assertion of requiredAssertions) {
  ...
  checks.push({
    name: `private beta intake docs state ${assertion.name}`,
    passed,
...
const schemaOrMigrationMarkers = [
  ...
  text
    .split(/\r?\n/)
    .flatMap((line, index) => (/phase 20|private beta round 1|feedback intake/i.test(line) ? [`${file}:${index + 1}`] : [])),
);

checks.push({
  name: "no Phase 20 DB schema or migration markers",
  passed: schemaOrMigrationMarkers.length === 0,
  detail: schemaOrMigrationMarkers.length === 0 ? "no Phase 20 schema/migration markers found" : schemaOrMigrationMarkers.join(", "),
});

const ownSource = readText("scripts/verify-private-beta-intake.ts");
...
checks.push({
  name: "private beta intake verifier is static and non-executing",
