...
export function CodexPromptHandoff({
  ...
  return (
    <section className="border border-sky-200/12 bg-[#0d1724]/78 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="border border-amber-200/20 bg-amber-200/10 px-2 py-1 text-[10px] font-semibold text-amber-100">
          Dry-run only
        </span>
      </div>
      ...
      <textarea
        readOnly
        value={prompt}
        className="mt-4 h-52 w-full resize-y border border-white/8 bg-black/24 p-3 font-mono text-xs leading-relaxed text-slate-200 outline-none"
      />

      <div className="mt-4 flex flex-wrap gap-3">
        <button onClick={copyPrompt} className="inline-flex items-center gap-2 border border-white/10 bg-white/[0.06] px-4 py-2 text-sm font-semibold text-slate-100 transition hover:bg-white/[0.1]">
