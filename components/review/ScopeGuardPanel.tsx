  ...
  return (
    <section className="min-w-0 border border-white/8 bg-[#0d1724]/78 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className={clsx("border px-2 py-1 text-[10px] font-semibold", scopeCheck ? statusStyles[scopeCheck.status] : "border-slate-200/12 bg-slate-200/6 text-slate-400")}>
          {scopeCheck ? statusLabels[scopeCheck.status] : "NOT RUN"}
        </span>
      </div>

      {!scopeCheck ? (
        <p className="mt-4 border border-white/[0.04] bg-black/12 px-3 py-2 text-xs text-slate-500">No scope check captured yet.</p>
      ) : (
            ...
            {scopeCheck.matchedFiles.length ? (
              <div className="mt-3 space-y-2">
                {scopeCheck.matchedFiles.slice(0, 8).map((filePath) => (
                  <p key={filePath} className="break-words border border-rose-200/10 bg-black/12 px-3 py-2 text-xs font-semibold text-rose-100">
                    {filePath}
                  </p>
                ))}
              </div>
            ) : (
              <p className="mt-3 border border-white/[0.04] bg-black/12 px-3 py-2 text-xs text-slate-500">No changed file paths matched forbidden scope rules.</p>
            )}
          </div>
        </div>
      )}

      <div className="mt-4 border border-rose-200/10 bg-rose-200/[0.035] p-3">
        <p className="text-xs font-bold text-rose-100">Forbidden Scope Rules</p>
        {rules.length ? (
          <ul className="mt-3 grid gap-2 md:grid-cols-2">
            {rules.map((rule) => (
              <li key={rule} className="border border-white/[0.04] bg-black/12 px-3 py-2 text-xs leading-relaxed text-slate-300">{rule}</li>
            ))}
          </ul>
        ) : (
          <p className="mt-3 border border-white/[0.04] bg-black/12 px-3 py-2 text-xs text-slate-500">No forbidden scope rules configured for this task.</p>
        )}
      </div>
