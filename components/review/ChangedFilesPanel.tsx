  ...
  return (
    <section className="min-w-0 border border-white/8 bg-[#0d1724]/78 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="border border-cyan-200/16 bg-cyan-200/8 px-2 py-1 text-[10px] font-semibold text-cyan-100">
          {fileChanges.length} total
        </span>
      </div>

      {fileChanges.length === 0 ? (
        <p className="mt-4 border border-white/[0.04] bg-black/12 px-3 py-2 text-xs text-slate-500">No changed files captured yet.</p>
      ) : (
        <div className="mt-4 grid min-w-0 gap-3 lg:grid-cols-2">
          {grouped.map((group) => (
            <div key={group.status} className="min-w-0 border border-white/[0.04] bg-white/[0.025] p-3">
              <p className="text-xs font-bold text-slate-200">{statusLabels[group.status]} ({group.items.length})</p>
              <div className="mt-3 space-y-2">
                {group.items.map((change) => (
                  <div key={change.id} className="border border-white/[0.04] bg-black/12 px-3 py-2 text-xs">
