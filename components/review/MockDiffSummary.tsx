...
export function MockDiffSummary({ task, review }: { task: Task; review?: ReviewRecord }) {
  return (
    <section className="border border-white/8 bg-[#0d1724]/64 p-4">
        ...
        <div>
          ...
          <div className="space-y-2">
            {task.changedFiles.map((file) => (
              <div key={file} className="flex items-center gap-2 border border-white/[0.04] bg-white/[0.035] px-3 py-2 text-sm text-slate-200">
        ...
        <div>
          ...
          <ul className="space-y-2">
            {(review?.diffSummary ?? ["No specific mock summary recorded for this task."]).map((item) => (
              <li key={item} className="border border-white/[0.04] bg-white/[0.03] px-3 py-2 text-sm leading-relaxed text-slate-300">
                {item}
              </li>
            ))}
          </ul>
