  ...
  return (
    <section className="border border-white/8 bg-[#0d1724]/78 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="border border-amber-200/16 bg-amber-200/8 px-2 py-1 text-[10px] font-semibold text-amber-100">
          Stats only
        </span>
      </div>

      {!diffSummary ? (
        <p className="mt-4 border border-white/[0.04] bg-black/12 px-3 py-2 text-xs text-slate-500">No diff summary captured yet.</p>
      ) : (
        <>
          ...
          <div className="mt-4 border border-white/[0.04] bg-white/[0.025] p-3">
            <p className="text-xs font-bold text-slate-200">Top Changed Files</p>
            {topRows.length ? (
              <div className="mt-3 space-y-2">
                {topRows.map((row) => (
                  <div key={`${row.rawLine}-${row.filePath}`} className="border border-white/[0.04] bg-black/12 px-3 py-2 text-xs">
              ...
              <p className="mt-3 border border-white/[0.04] bg-black/12 px-3 py-2 text-xs text-slate-500">No per-file stat rows captured.</p>
            )}
          </div>
        </>
      )}
