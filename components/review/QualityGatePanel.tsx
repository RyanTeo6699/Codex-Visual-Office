  ...
  return (
    <section className="border border-white/8 bg-[#0d1724]/64 p-4">
      ...
      <div className="mt-4 space-y-2">
        {checks.map((check) => (
          <div key={check.id} className="flex items-center justify-between gap-3 border border-white/[0.04] bg-white/[0.035] p-3">
            <div>
              <p className="text-sm font-semibold text-slate-100">{check.label}</p>
              <p className="mt-1 text-xs text-slate-500">{check.detail}</p>
            </div>
            <span className={clsx("border px-2.5 py-1 text-[11px] font-semibold", statusColor[check.status])}>
