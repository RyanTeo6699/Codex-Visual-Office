...
export function ApprovedProjectPathStatus({
  ...
  return (
    <section className="rounded-[22px] border border-white/8 bg-[#121a24]/72 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className={primaryPath ? "rounded-md border border-emerald-200/16 bg-emerald-200/8 px-2 py-1 text-[10px] font-semibold text-emerald-100" : "rounded-md border border-amber-200/14 bg-amber-200/8 px-2 py-1 text-[10px] font-semibold text-amber-100"}>
          {primaryPath ? "approved path exists" : "not configured"}
        </span>
      </div>
      ...
      {primaryPath ? (
        ...
        <p className="mt-4 rounded-[14px] border border-white/[0.04] bg-white/[0.025] px-3 py-2.5 text-xs leading-relaxed text-slate-500">
          No approved local path is saved for this project yet.
        </p>
      )}

      <Link href="/settings" className="mt-4 inline-flex rounded-[12px] border border-amber-200/14 bg-amber-200/8 px-3 py-2 text-xs font-bold text-amber-100 hover:bg-amber-200/12">
        Manage in Settings
      </Link>
    </section>
  );
